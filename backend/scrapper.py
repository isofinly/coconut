...
def fetch_url_with_headers(url: str):
...
def extract_keywords_multilang(text: str) -> Tuple[str, str, List[str]]:
    ...
    # Default to Russian model
    nlp = MODELS.get(detected_language, MODELS["ru"])
    doc = nlp(text)
...
def get_site_pages(url: str) -> Set[str]:
    ...
    """
    response = fetch_url_with_headers(url)
    if response.status_code != 200:
        return set([response.status_code])
...
def extract_paragraphs(url: str) -> List[str]:
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        paragraphs_lists = list(executor.map(
            process_paragraph, soup.find_all('p')))
...
def extract_metadata(url: str) -> Dict[str, Optional[str]]:
    ...
    if metadata['description'] and not metadata['keywords']:
        metadata['keywords'] = extract_keywords_multilang(
